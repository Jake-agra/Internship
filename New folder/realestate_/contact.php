...
if ($_POST && isset($_POST['submit_contact'])) {
    ...
    if (empty($namee) || empty($email)|| empty($message)) {
        ...
        try {
            ...
            // Try to send email
            if ($emailService->sendContactForm($namee, $email, $phone, $message)) {
                // Only save to database if email was sent successfully
                try {
                    $stmt = $conn->prepare("INSERT INTO contact(name, email, phone, message, created_at) VALUES (?, ?, ?, ?, NOW())");
                    $stmt->bind_param("ssss", $namee, $email, $phone, $message);
